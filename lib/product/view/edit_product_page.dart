...
class _EditProductPageState extends State<EditProductPage> {
  ...
  void didChangeDependencies() {
    ...
    if (!_initialized) {
      ...
      // Parse expired date
      if (_product.expired.isNotEmpty) {
        try {
          _expiredDate = DateTime.parse(_product.expired);
        } catch (_) {}
      }
  ...
  Future<void> _loadCategories() async {
    try {
      ...
      setState(() {
        ...
        _selectedCategory = data.firstWhere(
          (c) => c.id == _product.idCategory,
          orElse: () => data.isNotEmpty ? data.first : CategoryModel(id: 0, name: '', description: ''),
        );
